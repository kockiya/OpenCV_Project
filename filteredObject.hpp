...
bool updateFilteredObjectPosition(VideoCapture &cam, vector<filteredObject> &objects, int i)
{
	if(!cam.isOpened())
		return false;
...
bool drawFilteredObject(VideoCapture &cam, vector<filteredObject> &objects, int i, Mat &cameraFrame)
{
	if(!cam.isOpened())
		return false;
